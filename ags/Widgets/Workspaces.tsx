import Hyprland from "gi://AstalHyprland";
import Gtk from "gi://Gtk?version=4.0";
import { workspaceCount } from "./Settings";

export default function Workspaces() {
  const hypr = Hyprland.get_default();

  return (
    <box
      cssClasses={["workspaces-container"]}
      halign={Gtk.Align.CENTER}
      valign={Gtk.Align.CENTER}
      heightRequest={24}
      $={(self) => {
        let currentCount = 0;

        const rebuild = () => {
          const focusedWs = hypr.get_focused_workspace();

          const maxId =
            focusedWs && focusedWs.id > currentCount
              ? currentCount + 1
              : currentCount;

          // 🔥 Clear existing dots
          let child = self.get_first_child();
          while (child) {
            const next = child.get_next_sibling();
            self.remove(child);
            child = next;
          }

          // 🔥 Create dots
          for (let i = 1; i <= maxId; i++) {
            const dot = new Gtk.Box({
              css_classes: ["dot"],
              valign: Gtk.Align.CENTER,
            });

            const updateActive = () => {
              const fw = hypr.get_focused_workspace();
              if (!fw) return;

              // Normal case
              if (fw.id <= currentCount) {
                if (fw.id === i) dot.add_css_class("active");
                else dot.remove_css_class("active");
              } else {
                // Overflow case → last dot represents "beyond"
                if (i === maxId) dot.add_css_class("active");
                else dot.remove_css_class("active");
              }
            };

            const signalId = hypr.connect(
              "notify::focused-workspace",
              updateActive,
            );

            dot.connect("destroy", () => hypr.disconnect(signalId));

            updateActive();
            self.append(dot);
          }
        };

        const unsub = workspaceCount.subscribe((count: number) => {
          currentCount = count;
          rebuild();
        });

        const wsSignal = hypr.connect(
          "notify::focused-workspace",
          rebuild,
        );

        self.connect("destroy", () => {
          unsub();
          hypr.disconnect(wsSignal);
        });
      }}
    />
  );
}