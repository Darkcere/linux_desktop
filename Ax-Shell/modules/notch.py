...
class Notch(Window):
    def __init__(self, **kwargs):
        ...
        print(f"[{time.strftime('%H:%M:%S')}] Initializing Notch")

        self._typed_chars_buffer = ""
        self._launcher_transitioning = False
        self._launcher_transition_timeout = None
        self._last_window_icon_update = 0  # Debounce window icon updates
        ...
        # Check for an active player on startup
        if self.player_small.mpris_manager.get_players():
    ...
    def on_button_enter(self, widget, event):
        print(f"[{time.strftime('%H:%M:%S')}] Notch button enter: {widget.name}")
    ...
    def on_notch_hover_area_enter(self, widget, event):
        print(f"[{time.strftime('%H:%M:%S')}] Notch hover area enter")
        self.is_hovered = True
    ...
    def on_notch_hover_area_leave(self, widget, event):
        print(f"[{time.strftime('%H:%M:%S')}] Notch hover area leave")
        if event.detail == Gdk.NotifyType.INFERIOR:
            return False
        self.is_hovered = False
        return False
    ...
    def close_notch(self):
        print(f"[{time.strftime('%H:%M:%S')}] Closing notch")
    ...
    def _on_compact_scroll(self, widget, event):
        print(f"[{time.strftime('%H:%M:%S')}] Compact stack scroll event")
        if self._scrolling:
            return True
    ...
    def _build_app_identifiers_map(self):
        print(f"[{time.strftime('%H:%M:%S')}] Building app identifiers map")
        identifiers = {}
    ...
    def update_window_icon(self, *args):
        ...
        if conn:
            try:
                ...
                print(f"[{time.strftime('%H:%M:%S')}] Error updating window icon: {e}")
                try:
    ...
    def _check_occlusion(self):
        print(f"[{time.strftime('%H:%M:%S')}] Checking occlusion")
        occlusion_edge = "top"
        occlusion_size = 40
    ...
    def _get_current_window_class(self):
        ...
        try:
            ...
            print(f"[{time.strftime('%H:%M:%S')}] Error getting window class: {e}")
        return ""

    def on_active_window_changed(self, *args):
        print(f"[{time.strftime('%H:%M:%S')}] Active window changed")
        if data.PANEL_THEME != "Notch":
            return
    ...
    def _restore_occlusion_check(self):
        print(f"[{time.strftime('%H:%M:%S')}] Restoring occlusion check")
        self._prevent_occlusion = False
        self._occlusion_timer_id = None
        return False
