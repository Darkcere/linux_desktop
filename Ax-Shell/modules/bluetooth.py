...
import modules.icons as icons  # 👈 Your original icons module

# Simulated BluetoothDevice class with GObject signal
class BluetoothDevice(GObject.GObject):
    __gsignals__ = {
        "changed": (GObject.SIGNAL_RUN_FIRST, None, ()),
    }

    def __init__(self, name, icon_name, paired=False):
        super().__init__()
        self.name = name
    ...
    def set_connecting(self, value: bool):
        self.connecting = value
        if value:
            GObject.timeout_add(1000, self.finish_connection)
        self.emit("changed")
...
class BluetoothConnections(Box):
    ...
    def status_label(self):
        if self.client.enabled:
            ...
            for i in [
                self.bt_status_button, self.bt_status_text,
                self.bt_icon, self.bt_label,
                self.bt_menu_button, self.bt_menu_label
            ]:
